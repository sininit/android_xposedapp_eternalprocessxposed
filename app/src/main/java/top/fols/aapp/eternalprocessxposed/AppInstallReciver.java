		...
		if (TextUtils.equals(action, Intent.ACTION_PACKAGE_ADDED)) {
			try {
				...
				if (MainActivity.isNewInstallAutoAddLock(context)) {
					StrringConfig.lockMessage lockMessage = new StrringConfig.lockMessage();
					lockMessage.islock = true;
					lockMessage.packageName = packageName;
					StrringConfig.addPackageName(StrringConfig.getPackageNameListSharedPreferencesDirFileAndUpdatePermissions(context), lockMessage);
				} else
					return;
			...
			StrringConfig.lockMessage lockMessage = new StrringConfig.lockMessage();
			lockMessage.packageName = packageName;
			StrringConfig.removePackageList(StrringConfig.getPackageNameListSharedPreferencesDirFileAndUpdatePermissions(context), lockMessage);
		}
    }
}
