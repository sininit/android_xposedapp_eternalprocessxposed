...
import top.fols.box.lang.reflect.optdeclared.XReflectObjCmf;
...
//		}
	...
	public static Method getMethod(Set<XC_MethodHook.Unhook> hookAllMethods, Class[] clss) {
		...
		for (XC_MethodHook.Unhook un:hookAllMethods) {
			...
			if (XArrays2.arrayContentsEquals(method.getParameterTypes(), clss)) {
				method0 = method;
	...
	public static void setOomAdj(Object ProcessRecordObj, int newAdj) {
		try {
			...
			int adj = newAdj;
			try {
				XReflectObjCmf.defaultInstance.getObjMethod(ProcessListClass.get(),
															"setOomAdj",
															pid, adj)
					.invoke(null, pid, adj);
			} catch (Throwable e) {
	...
	public static void initHook(XC_LoadPackage.LoadPackageParam lpparam) {
		try {
			if (Config.android_PackageName.equals(lpparam.packageName)) {
				Hook.ProcessListClass.set(XposedHelpers.findClass(
				...
				ProcessList_PERSISTENT_SERVICE_ADJ.set(((Integer) getStaticFieldValue(Hook.ProcessListClass.get(), "PERSISTENT_SERVICE_ADJ", ProcessList_DEFAULT_ADJ.get())).intValue());
			...
			if (Config.android_PackageName.equals(lpparam.packageName)) {
				...
				XposedBridge.hookAllMethods(ActivityManagerServiceClass, "newProcessRecordLocked",
					new XC_MethodHook() {
						@Override
						protected void afterHookedMethod(MethodHookParam param) throws Throwable {
							ApplicationInfo applicationInfo = (ApplicationInfo) findParamObject(param.args, ApplicationInfo.class);
							...
							if (lock || lockEternal) {
								...
								try {
									setFieldValue(ProcessRecord, "maxAdj", adj);
								} catch (Exception e) {
									log(e);
								}
						...
						protected void beforeHookedMethod(MethodHookParam param) throws Throwable {
							try {
								...
								if (lock || lockEternal) {
									...
									try {
										Hook.setOomAdj(ProcessRecord, adj);
									} catch (Exception e) {
										log(e);
									}
								}
